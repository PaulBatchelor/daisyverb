...
static HAL_StatusTypeDef send_cmd(I2C_HandleTypeDef *hi2c, uint8_t reg, uint8_t byte);

void codec_pcm3060_init(I2C_HandleTypeDef *hi2c)
{
	// Crude as heck
	// Ideal Register 64	
	// 7	6		5		4		3	2	1	0
...
//	}
	uint8_t cmd;
	cmd = 0 | (FMT_I2S);
	send_cmd(hi2c, REGISTER_ADC_CTL, cmd);
	send_cmd(hi2c, REGISTER_DAC_CTL, cmd);
