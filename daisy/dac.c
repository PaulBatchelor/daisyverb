...
void dsy_dac_init(dsy_dac_handle_t *dsy_hdac,
                  dsy_dac_channel channel)
{
	...
	if(channel == DSY_DAC_CHN1 ||
       channel == DSY_DAC_CHN_BOTH) {
		/* Init Channel 1 */
...
void HAL_DAC_MspInit(DAC_HandleTypeDef *hdac)
{
	...
	if(hdac->Instance == DAC1) {
		...
		if(dac.active_channels == DSY_DAC_CHN2
		   || dac.active_channels == DSY_DAC_CHN_BOTH) {
			port = (GPIO_TypeDef *)
                (gpio_hal_port_map[dac.dsy_hdac->pin_config[DSY_DAC_CHN2].port]);
			ginit.Pin =
