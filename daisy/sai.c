...
void dsy_sai_init_from_handle(dsy_sai_handle_t* hsai)
{
	...
	switch(dsy_sai_handle.init) {
		case DSY_AUDIO_INIT_SAI1:
            dsy_sai1_init();
            break;
		case DSY_AUDIO_INIT_SAI2: dsy_sai2_init();
            break;
		case DSY_AUDIO_INIT_BOTH:
			dsy_sai1_init();
			dsy_sai2_init();
...
void HAL_SAI_MspInit(SAI_HandleTypeDef* hsai)
{
	...
	if (hsai->Instance == SAI1_Block_A) {
		...
		for (i = 0; i < DSY_SAI_PIN_LAST; i++) {
			...
			if (i != DSY_SAI_PIN_MCLK ||
                dsy_sai_handle.sync_config[DSY_SAI_1] ==
                DSY_AUDIO_SYNC_MASTER) {
				port = (GPIO_TypeDef*)
	...
	if (hsai->Instance == SAI2_Block_B) {
		...
		for (i = 0; i < DSY_SAI_PIN_LAST; i++) {
            ...
            rc = i != DSY_SAI_PIN_MCLK ||
                dsy_sai_handle.sync_config[DSY_SAI_2] ==
                DSY_AUDIO_SYNC_MASTER;
			...
			if (rc) {
				...
				GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
				GPIO_InitStruct.Alternate
					= i == DSY_SAI_PIN_SCK ? GPIO_AF8_SAI2 :
                    GPIO_AF10_SAI2;
				HAL_GPIO_Init(port, &GPIO_InitStruct);
			}
		}
